...
## Model F — zero-float relay (planned, second product line)
...
| Pricing mechanism | Open — commission (pass the upstream rate through, earn a referral fee) or margin (quote your own rate, position as embedded infra the way Model D is). Gated at build chunk R2. |
| Reuses from Model D | C1 (ledger, new account tree), C3 (screening, unchanged), C4 (energy broker, smaller job), S1 (key management, smaller blast radius) |
| New | A TRON-side deposit watcher (C2′, mirroring C2 against TRC20), a relay forwarder service (`relayd`), and `internal/upstream` — a vendor-agnostic interface to whichever instant-exchange platform is chosen |
| Build status | Not started. Docs only. |
| Next action | R2 in the build-prompts doc: get a real quote/create-order/status API and a partner/affiliate terms sheet from a candidate upstream platform before any `relayd` code is written against vendor-specific assumptions. |
