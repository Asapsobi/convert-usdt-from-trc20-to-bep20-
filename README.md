...
just left as an unbacked credit). Not yet built: the refund path for a
`HELD`/`FAILED`/`EXPIRED` upstream order (R5 — logged loudly today, no
automatic recovery), a stale-relay-leg reconciliation alarm, and relayd's own
replay ship-gate harness (R6). `cmd/relayd` is the binary — see its own
