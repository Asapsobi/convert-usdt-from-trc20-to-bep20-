...
just left as an unbacked credit). `internal/orchestrate/refund.go` is R5's own
first slice: a leg stuck unable to broadcast its own forward transfer past
...
no new broadcast code, only a new trigger. `internal/orchestrate/reconcile.go` is the
...
rather than querying C1's journal directly — deliberately excludes `AWAITING_DEPOSIT`,
the same gap refund.go already flags for that one status (see reconcile.go's own doc
comment). `internal/replay` + `cmd/replay` is R6's own ship gate, connecting to a real,
already-running `ledgerd` (the same convention every sibling `cmd/replay` in this repo
already uses) — six scenarios (both directions' happy path, the automatic timeout
refund, the manually-rejected-hold refund, `UNRECOVERABLE`-with-alert, and the stale-leg
alarm), run for real across several seeds, all `ALL PASSED`.
Not yet built: a leg stuck `AWAITING_DEPOSIT` because `upstream.CreateOrder` itself
never succeeds, and real per-vendor `EXPIRED` semantics (gated on a real vendor, R2/R4).
